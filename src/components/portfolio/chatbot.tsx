...
import { Bot, Send, X, CornerDownLeft, Loader2 } from 'lucide-react';
...
export function Chatbot({ role }: { role: Role }) {
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 sm:right-8 w-[calc(100%-2rem)] max-w-sm h-[60vh] bg-card border rounded-lg shadow-2xl flex flex-col z-50"
          >
      ...
      <Button
        size="icon"
        className="fixed bottom-4 right-4 sm:right-8 w-16 h-16 rounded-full shadow-lg z-50"
